...
class ClientiFurnizori {
    ...
    public function read_parentchilds() {
        $query = 'SELECT clienti.keyid as keyid_childs, :keyid_parent as keyid_parent, clienti.nume
                        , CASE WHEN furnizori_clienti.keyid_furnizori = :keyid_parent THEN 1 ELSE 0 END as isassociated
                  FROM clienti
                  left JOIN (SELECT keyid_clienti, keyid_furnizori
                             FROM ' . $this->table . '
                            WHERE keyid_furnizori = :keyid_parent
                            ) furnizori_clienti ON furnizori_clienti.keyid_clienti = clienti.keyid
    ...
    public function set_parentchilds() {
        $query = ' CALL manage_clienti_furnizori(:keyid_parent, :keyid_childs); ';
    ...
    public function update() {
        $query = 'UPDATE ' . $this->table . ' SET keyid_clienti = :keyid_childs, keyid_furnizori = :keyid_parent WHERE keyid = :keyid';
        $stmt = $this->conn->prepare($query);
    ...
    public function filter_users($keyid_childs, $keyid_parent, $start, $limit) {
        $query = 'SELECT keyid, keyid_clienti, keyid_furnizori FROM ' 
                . $this->table 
                . ' WHERE keyid_clienti =:keyid_childs AND keyid_furnizori = :keyid_parent LIMIT :start, :limit';
        $stmt = $this->conn->prepare($query);
    ...
    public function count_filtered($keyid_childs, $keyid_parent) {
        $query = 'SELECT COUNT(*) as total FROM ' . $this->table . ' WHERE keyid_clienti = :keyid_childs AND keyid_furnizori = :keyid_parent';
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':keyid_childs', $keyid_childs, PDO::PARAM_STR);
        $stmt->bindParam(':keyid_parent', $keyid_parent, PDO::PARAM_STR);
        $stmt->execute();
